...
class Hafas(commands.Cog):
        ...
        async with HafasClient() as client:
            ...
            if not client_status.desktop or client_status.web:
                ...
                for departure in departures:
                    text += f"{departure.time.strftime('%H:%M')} {departure.destination} {departure.delay}\n"
            else:
    ...
    async def departures(self, ctx: "Context", amount: int, *, name: str) -> None:
        if not 0 < amount < 25:
            await ctx.reply("You must provide a number from 1 to 24")
            return
    ...
    async def arrivals(self, ctx: "Context", amount: int, *, name: str) -> None:
        if not 0 < amount < 25:
            await ctx.reply("You must provide a number from 1 to 24")
            return
    ...
    async def journey(self, ctx: "Context", *, string: str) -> None:
        try:
            departure, arrival = string.split("|")
        except ValueError:
            await ctx.reply("You did not provide two stations separated by |")
            return
        ...
        async with HafasClient() as client:
            ...
            for journey in journeys:
                ...
                embed.add_field(name="Delay", value=str(journey.times.delay // 60) if journey.times.delay else "OK", inline=True)
                embed.add_blank_field()
